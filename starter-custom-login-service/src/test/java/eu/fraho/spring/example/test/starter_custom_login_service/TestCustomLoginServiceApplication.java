 ...
 */
package eu.fraho.spring.example.test.starter_custom_login_service;
...
public class TestCustomLoginServiceApplication {
    ...
    private String obtainToken() throws Exception {
        MockHttpServletRequestBuilder req = MockMvcRequestBuilders.post("/auth/login")
                .contentType(MediaType.APPLICATION_JSON)
                .content(String.format("{\"username\":\"%s\",\"password\":\"%s\"}", "foo", "obviously wrong as the new login service doesn't check the password :)"))
                .accept(MediaType.APPLICATION_JSON);
